...
#include <snax/ibc_plugin/ibc_plugin.hpp>
#include <snax/ibc_plugin/protocol.hpp>
...
{
   auto actn = get_action(account, N(chaininit), vector<permission_level>{{my_impl->relay, config::active_name}}, mvo()("header", fc::raw::pack(msg.header))("active_schedule", msg.active_schedule)("blockroot_merkle", msg.blockroot_merkle));
...
{
   auto actn = get_action(account, N(pushsection), vector<permission_level>{{my_impl->relay, config::active_name}}, mvo()("headers", fc::raw::pack(msg.headers))("blockroot_merkle", msg.blockroot_merkle)("relay", my_impl->relay));
...
{
   auto actn = get_action(account, N(blockmerkle), vector<permission_level>{{my_impl->relay, config::active_name}}, mvo()("block_num", data.block_num)("merkle", data.merkle)("relay", my_impl->relay));
...
{
   auto actn = get_action(account, N(rmfirstsctn), vector<permission_level>{{my_impl->relay, config::active_name}}, mvo()("relay", my_impl->relay));
...
void ibc_token_contract::push_cash_recurse(int index, const std::shared_ptr<std::vector<cash_action_params>> &params, uint32_t start_seq_num)
{
   ...
   auto actn = get_action(account, N(cash), vector<permission_level>{{my_impl->relay, config::active_name}}, mvo()("seq_num", start_seq_num)("orig_trx_block_num", par.orig_trx_block_num)("orig_trx_packed_trx_receipt", par.orig_trx_packed_trx_receipt)("orig_trx_merkle_path", par.orig_trx_merkle_path)("orig_trx_id", par.orig_trx_id)("to", par.to)("quantity", par.quantity)("memo", par.memo)("relay", my_impl->relay));
...
void ibc_token_contract::push_cashconfirm_recurse(int index, const std::shared_ptr<std::vector<cashconfirm_action_params>> &params)
{
   ...
   auto par = params->at(index);
   auto actn = get_action(account, N(cashconfirm), vector<permission_level>{{my_impl->relay, config::active_name}}, mvo()("cash_trx_block_num", par.cash_trx_block_num)("cash_trx_packed_trx_receipt", par.cash_trx_packed_trx_receipt)("cash_trx_merkle_path", par.cash_trx_merkle_path)("cash_trx_id", par.cash_trx_id)("orig_trx_id", par.orig_trx_id));
...
{
   auto actn = get_action(account, N(cash), vector<permission_level>{{my_impl->relay, config::active_name}}, mvo()("seq_num", p.seq_num)("orig_trx_block_num", p.orig_trx_block_num)("orig_trx_packed_trx_receipt", p.orig_trx_packed_trx_receipt)("orig_trx_merkle_path", p.orig_trx_merkle_path)("orig_trx_id", p.orig_trx_id)("to", p.to)("quantity", p.quantity)("memo", p.memo)("relay", p.relay));
...
{
   auto actn = get_action(account, N(cashconfirm), vector<permission_level>{{my_impl->relay, config::active_name}}, mvo()("cash_trx_block_num", p.cash_trx_block_num)("cash_trx_packed_trx_receipt", p.cash_trx_packed_trx_receipt)("cash_trx_merkle_path", p.cash_trx_merkle_path)("cash_trx_id", p.cash_trx_id)("orig_trx_id", p.orig_trx_id));
...
void ibc_token_contract::push_rborrm_recurse(int index, const std::shared_ptr<std::vector<transaction_id_type>> &params, name action_name)
{
   ...
   auto actn = get_action(account, action_name, vector<permission_level>{{my_impl->relay, config::active_name}}, mvo()("trx_id", trx_id)("relay", my_impl->relay));
...
void ibc_plugin_impl::irreversible_block(const block_state_ptr &block)
{
   ...
   if (blockroot_merkle_cache.size() > 3600 * BlocksPerSecond * 24 * 7)
   { // one week
...
void ibc_plugin_impl::handle_message(connection_ptr c, const ibc_heartbeat_message &msg)
{
   ...
   ilog("received msg: origtrxs_table_id_range [${of},${ot}] cashtrxs_table_seq_num_range [${cf},${ct}] new_producers_block_num ${n}, lwcls_range [${lsf},${lst}]",
        ("of", msg.origtrxs_table_id_range.first)("ot", msg.origtrxs_table_id_range.second)("cf", msg.cashtrxs_table_seq_num_range.first)("ct", msg.cashtrxs_table_seq_num_range.second)("n", msg.new_producers_block_num)("lsf", msg.lwcls.first_num)("lst", msg.lwcls.last_num));
   ...
   { // validate and update local lwcls_info
...
void ibc_plugin_impl::handle_message(connection_ptr c, const lwc_section_data_message &msg)
{
   ...
   if (msg_first_num == ls.last + 1)
   { // append directly
      chain_contract->pushsection(msg);
   }

   else if (msg_first_num <= ls.last)
   { // find fit number then append directly // todo review
   ...
   if (msg.table == N(origtrxs))
   {
      for (auto id = msg.range.first; id <= msg.range.second && id <= msg.range.first + max_responses_per_time; ++id)
      {
         ...
         if (p.valid())
         {
            ...
            if (trx_info.block_time_slot >= safe_tslot)
            {
               break;
            }
            ...
            for (auto &c : connections)
            {
               if (c->current())
               {
                  ...
                  dlog("origtrxs_table_id_range [${of},${ot}] cashtrxs_table_seq_num_range [${cf},${ct}] new_producers_block_num ${n}, lwcls_range [${lsf},${lst},${v}]",
                       ("of", msg.origtrxs_table_id_range.first)("ot", msg.origtrxs_table_id_range.second)("cf", msg.cashtrxs_table_seq_num_range.first)("ct", msg.cashtrxs_table_seq_num_range.second)("n", msg.new_producers_block_num)("lsf", msg.lwcls.first_num)("lst", msg.lwcls.last_num)("v", msg.lwcls.valid));
...
void ibc_plugin_impl::ibc_core_checker()
{
   ...
   {
      min_last_num = std::max(min_last_num, new_prod_blk_nusnax::cheduleReplaceMinLength + chain_contract->lwc_lib_depth);
   }

   // check if lwcls reached the minimum range, if not, send lwc_section_request_message
   bool reached_min_lsnax::true;
   ...
   if (range.first == 0)
   { // range.first == 0 means cashtrxs is empty, range.second shoule alse be 0
   ...
   else
   {
      ...
      if (cash_opt.valid())
      {
         ...
         { // maybe happen when restart ibc_plugin node
      ...
      {
         ilog("internal error, failed to get cash transaction information of seq_num ${n}", ("n", range.second));
      }
   }
...
{
   cfg.add_options()("ibc-chain-contract", bpo::value<string>(), "Name of this chain's ibc chain contract")("ibc-token-contract", bpo::value<string>(), "Name of this chain's ibc token contract")("ibc-relay-name", bpo::value<string>(), "ID of relay controlled by this node (e.g. relayone)")("ibc-relay-private-key", bpo::value<string>(),
       ...
       ("ibc-listen-endpoint", bpo::value<string>()->default_value("0.0.0.0:5678"), "The actual host:port used to listen for incoming ibc connections.")("ibc-server-address", bpo::value<string>(), "An externally accessible host:port for identifying this node. Defaults to ibc-listen-endpoint.")("ibc-sidechain-id", bpo::value<string>(), "The sidechain's chain id")("ibc-peer-address", bpo::value<vector<string>>()->composing(), "The public endpoint of a peer node to connect to. Use multiple ibc-peer-address options as needed to compose a network.")("ibc-max-nodes-per-host", bpo::value<int>()->default_value(def_max_nodes_per_host), "Maximum number of client nodes from any single IP address")("ibc-allowed-connection", bpo::value<vector<string>>()->multitoken()->default_value({"any"}, "any"), "Can be 'any' or 'specified' or 'none'. If 'specified', peer-key must be specified at least once.")("ibc-peer-key", bpo::value<vector<string>>()->composing()->multitoken(), "Optional public key of peer allowed to connect.  May be used multiple times.")("ibc-agent-name", bpo::value<string>()->default_value("\"EOSIO IBC Agent\""), "The name supplied to identify this node amongst the peers.")("ibc-peer-private-key", bpo::value<vector<string>>()->composing()->multitoken(),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      "Key=Value pairs in the form <public-key>=KEY:<private-key>\n"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      "   <public-key>   \tis a string form of a vaild EOSIO public key\n\n"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      "   <private-key>  \tis a string form of a valid EOSIO private key which maps to the provided public key\n\n")("ibc-max-clients", bpo::value<int>()->default_value(def_max_clients), "Maximum number of clients from which connections are accepted, use 0 for no limit")("ibc-connection-cleanup-period", bpo::value<int>()->default_value(def_conn_retry_wait), "Number of seconds to wait before cleaning up dead connections")("ibc-max-cleanup-time-msec", bpo::value<int>()->default_value(10), "Maximum connection cleanup time per cleanup call in millisec")("ibc-version-match", bpo::value<bool>()->default_value(false), "True to require exact match of ibc plugin version.")
...
void ibc_plugin::plugin_initialize(const variables_map &options)
{
   ...
   try
   {
      ...
      my->chain_contract.reset(new ibc_chain_contract(snax::chain::name{options.at("ibc-chain-contract").as<string>()}));
      ilog("ibc chain contract account is ${name}", ("name", options.at("ibc-chain-contract").as<string>()));

      OPTION_ASSERT("ibc-token-contract")
      my->token_contract.reset(new ibc_token_contract(snax::chain::name{options.at("ibc-token-contract").as<string>()}));
      ilog("ibc token contract account is ${name}", ("name", options.at("ibc-token-contract").as<string>()));

      OPTION_ASSERT("ibc-relay-name")
      my->relay = snax::chain::name{options.at("ibc-relay-name").as<string>()};
